...
public class InscripcionesModel {
	...
	public static final String SQL_LISTA_INSCRIPCIONES=
			"SELECT *"
			+" from Inscripciones  where fechaInscripcion>=? order by id";
	/**
	 ...
	 */
	public List<Object[]> getListaInscripcionesArray(Date fechaInscripcion) {
		validateNotNull(fechaInscripcion,MSG_FECHA_INSCRIPCION_NO_NULA);
		//concatena los campos deseados en una unica columna pues el objetivo es devolver una lista de strings
		String sql="SELECT id || '-' || idColegiado || '-' || idCurso || '-' || fechaInscripcion from (" + SQL_LISTA_INSCRIPCIONES + ")";
	 ...
	 */
	public List<InscripcionesDisplayDTO> getListaInscripciones(Date fechaInscripcion) {
		validateNotNull(fechaInscripcion,MSG_FECHA_INSCRIPCION_NO_NULA);
		String sql=
				 "SELECT"
				 + "    i.id,"
				 + "    c.nombre as nombreColegiado,"
				 + "    c.apellido as apellidoColegiado,"
				 + "    cr.titulo_curso as tituloCurso,"
				 + "    i.fechaInscripcion as fechaInscripcion"
				 + " FROM Inscripciones AS i"
				 + " JOIN Colegiados AS c ON c.id = i.idColegiado"
				 + " JOIN Cursos AS cr ON cr.id = i.idCurso"
				 + " ORDER BY i.id";
		String d=Util.dateToIsoString(fechaInscripcion);
		return db.executeQueryPojo(InscripcionesDisplayDTO.class, sql);
	}
	/** 
	 ...
	 */
	public InscripcionesEntity getInscripcion(int id) {
		String sql="SELECT * from Inscripciones where id=?";
		List<InscripcionesEntity> inscripciones=db.executeQueryPojo(InscripcionesEntity.class, sql, id);
		validateCondition(!inscripciones.isEmpty(),"Id de inscripcion no encontrado: "+id);
	...
	public void updateFechasInscripcion(int id, Date inicio, Date fin) {
		InscripcionesEntity inscripcion=this.getInscripcion(id);
		validateFechasInscripcion(inicio, fin, Util.isoStringToDate(inscripcion.getFechaInscripcion()));
